...
all_data['is_extreme'] = (all_data['precipitation_sum'] >= 50).astype(int)
yearly_data = all_data.groupby(['city', 'year'])['is_extreme'].sum().reset_index()

heatmap_data = yearly_data.pivot(index='city', columns='year', values='is_extreme').fillna(0).astype(int)
...
sns.heatmap(heatmap_data, cmap=mcolors.LinearSegmentedColormap.from_list('WhiteBlues', ['white', 'lightblue', 'blue', 'darkblue']), ax=ax, annot=annot_data, fmt="", annot_kws={"size": 8},
            linewidths=0.1, linecolor='lightgray', xticklabels=True, cbar_kws={'label': 'Giorni >= 50 mm'})

ax.set_title('Numero di giorni con precipitazioni estreme (>= 50 mm) per anno in Europa', fontsize=18)
