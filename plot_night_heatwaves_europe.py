...
def compute_heatwaves(city_file):
    ...
    baseline_data = df[(df['date'].dt.year >= 1991) & (df['date'].dt.year <= 2020) & (df['date'].dt.month.isin([6, 7, 8]))]['temperature_2m_min'].dropna()
    ...
    df['is_hot'] = df['temperature_2m_min'] > df['threshold']
        ...
        if row['is_hot']:
            if len(current_hw) > 0 and (row['date'] - current_hw[-1]['date']).days > 1:
                if len(current_hw) >= 6:
                    ...
                    max_temp = hw_df['temperature_2m_min'].max()
...
def plot_waves(cities, output_file, region_title):
    ...
    # Sync events logic
    sync_dates = [d for d, count in date_counts.items() if count >= len(cities) * 0.25]
    sync_events = []
    if sync_dates:
        sync_dates.sort()
...
if __name__ == "__main__":
    ...
    out_path = 'docs/night_heatwaves_europe.png'
