...
# Precipitazione massima in 24h per anno e per citta
yearly_data = all_data.groupby(['city', 'year'])['precipitation_sum'].max().reset_index()

# Calculate records
yearly_data = yearly_data.sort_values(by=['city', 'year'])
yearly_data['prev_max'] = yearly_data.groupby('city')['precipitation_sum'].transform(lambda x: x.cummax().shift(1))

# A record is when the current rain is strictly greater than all previous ones
yearly_data['is_record'] = (yearly_data['precipitation_sum'] > yearly_data['prev_max']).astype(int)
...
heatmap_rain = yearly_data.pivot(index='city', columns='year', values='precipitation_sum')

heatmap_data['latitude'] = heatmap_data.index.map(latitudes)
heatmap_data = heatmap_data.sort_values('latitude', ascending=False)
heatmap_rain['latitude'] = heatmap_rain.index.map(latitudes)
heatmap_rain = heatmap_rain.sort_values('latitude', ascending=False)
...
heatmap_rain = heatmap_rain.drop(columns=['latitude'])

annot_data = np.where(heatmap_data == 1, heatmap_rain.round(1).astype(str), "")

fig, ax = plt.subplots(figsize=(30, 12))
# Custom colormap: -1=LightGray, 0=White, 1=Light blue
cmap = mcolors.ListedColormap(['lightgray', 'white', '#7ec8e3'])
...
for i, year in enumerate(heatmap_data.columns):
    if year in years_to_highlight:
        ...
        ax.add_patch(plt.Rectangle((i, 0), 1, heatmap_data.shape[0], fill=False, edgecolor='blue', lw=3, zorder=10))
...
ax.set_title('Nuovi Record di Precipitazione Giornaliera nei Capoluoghi Italiani (1940-2026) [Primi 15 anni usati come storico]', fontsize=18)
...
output_path = '/Users/lcozzuto/git/meteo/docs/record_heatmap_rain_italy.png'
plt.savefig(output_path, dpi=300, bbox_inches='tight')
print(f"Rain records heatmap saved to {output_path}")
